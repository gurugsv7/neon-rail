import {createCapture,CAPTURE_DURATION} from './capture.js';
import {BODY_LINKS} from './body-signal.js';
...
import {LANE,MAX_SPEED,TRAIN_ROOF,TRAIN_CLEARANCE,TRAIN_HALF,RAMP_LENGTH,PlayerController,SaveManager,MISSIONS,POWER_NAMES,planPattern,difficultyAt,rng} from './core.js';
...
class WorldGenerator{
  ...
  generate(distance){while(this.next<distance+310){const pattern=planPattern(this.index,this.safe,this.random,this.next),s=this.next;
      for(const h of pattern.hazards){this.addEntity(h.type,h.lane,s).incomingSpeed=pattern.incomingSpeed;if(['rampTrain','train'].includes(h.type)){for(let z=-5;z<=5;z+=2.5)this.addToken(h.lane,s+z,TRAIN_ROOF+.95);if(h.type==='rampTrain')for(let j=0;j<7;j++)this.addToken(h.lane,s-TRAIN_HALF-RAMP_LENGTH+j*2.1,1+j*2.1/RAMP_LENGTH*TRAIN_ROOF);}
        else if(h.type==='hurdle'||h.type==='cargo'||h.type==='gap'){for(let j=-2;j<=2;j++)this.addToken(h.lane,s+j*2.1,1.2+Math.cos(j*.55)*1.8);}}
      // Corridor fillers, plus anything standing in the gate's escape lane.
      // Every one is telegraphed by its tokens: they arc to the jump apex over
      // a hurdle and drop to slide height under a sign, so the input is read
      // off the track rather than memorised.
      const reflexes=[...pattern.fillers,...(pattern.gate?[pattern.gate]:[])];
      for(const f of reflexes){const z=s+f.offset;this.addEntity(f.type,f.lane,z);
        if(f.type==='hurdle')for(let j=-2;j<=2;j++)this.addToken(f.lane,z+j*2.1,1.2+Math.cos(j*.55)*1.8);
        else for(let j=-1;j<=1;j++)this.addToken(f.lane,z+j*2.1,.55);}
      // A visible token ribbon traces the guaranteed bypass lane, broken where
      // a filler stands in it so the ribbon never reads through an obstacle.
      const blocked=reflexes.filter(f=>f.lane===pattern.safe).map(f=>s+f.offset);
      for(let j=-3;j<=4;j++){const z=s+j*3.3;if(!blocked.some(b=>Math.abs(b-z)<2.6))this.addToken(pattern.safe,z);}
      const old=this.safe;for(let j=0;j<4;j++){const lane=j<2?old:pattern.safe;this.addToken(lane,s-27+j*3.7);}
      if(this.index% (pattern.pressure<.18?3:pattern.pressure<.5?4:5)===0)this.addPower(['magnet','shield','boost','board'][Math.floor(this.index/2)%4],pattern.safe,s-13);
  ...
  step(dt,game){const distance=game.distance;this.generate(distance);
    for(const c of this.chunks){if(c.index*48+24<distance-42)c.index+=this.chunks.length;c.group.position.z=distance-c.index*48;c.group.visible=c.group.position.z>-190&&c.group.position.z<60;}
    for(let i=this.entities.length-1;i>=0;i--){const e=this.entities[i],ahead=e.s-distance;
      ...
      if(e.type==='approach'&&(e.age>0||e.baseS-distance<50)){e.age+=dt;e.s-=(e.incomingSpeed||18)*dt;}
      if(e.type==='depart'&&(e.age>0||e.baseS-distance<40)){e.age+=dt;e.s+=2.5*dt;}
      e.model.visible=ahead<180;e.model.position.set((e.lane-1)*LANE,0,distance-e.s);if(e.ramp)e.ramp.position.set((e.lane-1)*LANE,0,distance-e.s+TRAIN_HALF+RAMP_LENGTH/2);
      if(e.type==='approach'&&!e.warned&&ahead<78){e.warned=true;game.audio.play('horn');if(game.state==='playing')game.iconToast('train','!',1.2,'Incoming train');}
    ...
    for(let i=this.powers.length-1;i>=0;i--){const p=this.powers[i];p.model.position.set(p.x,p.y+Math.sin(game.time*3)*.12,distance-p.s);p.model.rotation.y=Math.sin(game.time*1.5)*.2;if(p.s<distance-8){this.scene.remove(p.model);this.powers.splice(i,1);}}
  }
...
class UIManager{
  constructor(game){this.game=game;this.lastUpdate=0;this.powerSignature='';document.querySelectorAll('[data-icon]').forEach(el=>el.innerHTML=icon(el.dataset.icon));
    const bind=(id,fn)=>$(id).addEventListener('click',()=>{game.audio.start();game.audio.play('ui');fn();document.activeElement?.blur();});
    bind('run',()=>game.start());bind('again',()=>game.start());bind('restart',()=>game.start());bind('resume',()=>game.resume());bind('quit',()=>game.menu());bind('main-menu',()=>game.menu());bind('pause-button',()=>game.pause());bind('audio',()=>{game.audio.enabled=!game.audio.enabled;$('audio').innerHTML=icon(game.audio.enabled?'sound':'muted');$('audio').setAttribute('aria-pressed',String(!game.audio.enabled));});bind('fullscreen',()=>game.fullscreen());this.state('menu');
  }
  ...
  update(dt){this.lastUpdate+=dt;if(this.lastUpdate<.07)return;this.lastUpdate=0;const g=this.game;$('score').textContent=String(Math.floor(g.score)).padStart(6,'0');$('distance').textContent=`${Math.floor(g.distance)} m`;$('tokens').lastElementChild.textContent=g.tokens;$('mult').textContent=`×${g.multiplier*(g.power.boost>0?2:1)}`;$('speedbar').style.width=`${(g.speed-12)/(MAX_SPEED-12)*100}%`;
    const zones=['SUNWASH YARDS','MERIDIAN STATION','GARDEN VIADUCT','SERVICE TUNNELS'],zone=Math.floor(g.distance/144)%4;if(this.zone!==zone){this.zone=zone;$('zone').innerHTML=zones.map((name,i)=>`<i class="${i===zone?'current':''}"></i>`).join('');$('zone').setAttribute('aria-label',zones[zone]);$('zone').title=zones[zone];}
    $('chase').setAttribute('aria-label',g.idleSeconds>0?'Jog! Security closing in':g.threat>0?'Security closing in':'Security signal low');$('chase').title=g.idleSeconds>0?'Jog! Security closing in':g.threat>0?'Security closing in':'Security signal low';$('chase').classList.toggle('danger',g.threat>0||g.idleSeconds>0);$('help').classList.toggle('hidden',g.distance>65);
    const active=Object.keys(g.power).filter(k=>g.power[k]>0),sig=active.join();
    if(sig!==this.powerSignature){this.powerSignature=sig;$('powers').innerHTML=active.map(k=>`<div class="power" id="power-${k}" role="img" style="--accent:#${POWER_COLORS[k].toString(16)}"><div class="powerface">${icon(k)}<span></span></div></div>`).join('');}
    for(const k of active){const el=$(`power-${k}`),seconds=Math.ceil(g.power[k]);el.querySelector('span').textContent=seconds+'s';el.style.setProperty('--remaining',`${g.power[k]/g.powerDuration[k]*100}%`);el.classList.toggle('expiring',seconds<=3);el.title=`${POWER_NAMES[k]} · ${seconds}s${k==='shield'?' · one hit covered':''}`;el.setAttribute('aria-label',el.title);}
...
class Game{
  constructor(){this.state='menu';this.time=0;this.distance=0;this.speed=17;this.score=0;this.tokens=0;this.threat=0;this.glance=0;this.seed=0;this.power={magnet:0,boost:0,shield:0,board:0};this.powerDuration={magnet:12,boost:14,shield:25,board:16};this.save=new SaveManager(localStorage);this.audio=new AudioManager();this.player=new PlayerController();this.collision=new CollisionSystem();this.shake=0;this.toastTime=0;this.accum=0;this.pendingDeath=0;this.clock=0;this.chain=0;this.lastToken=0;this.saveTimer=0;this.multiplier=1+MISSIONS.filter(m=>this.save.data[m.key]>=m.target).length;this.completed=new Set(MISSIONS.filter(m=>this.save.data[m.key]>=m.target).map(m=>m.key));
    ...
    this.world=new WorldGenerator(this.scene);this.menuTrain=this.world.templates.get('train').clone();this.menuTrain.position.set(-3.25,0,-11);this.scene.add(this.menuTrain);this.rig=characterModel();this.character=this.rig||courier();this.scene.add(this.character.root);this.drone=drone();this.scene.add(this.drone);this.capture=createCapture(this.scene);
  ...
  motion(){
    ...
    const WORDS={off:'Camera off',starting:'Requesting camera…',loading:'Loading tracker…',
      calibrating:'Hold still — centring',searching:'Step into view',tracking:'Lean to steer',view:'Show your full body',back:'Step back / tilt camera down',forward:'Move a little closer',left:'Move left',right:'Move right',still:'Hold still to calibrate',jog:'Jog in place to start',running:'Jogging detected'};
    const control=createMotionControl(this,(status,detail,head)=>{
      ...
      panel.classList.toggle('hidden',status==='off');
      panel.classList.toggle('body-mode',control.mode==='body');
      $('body-mode').setAttribute('aria-pressed',String(control.mode==='body'));$('head-mode').setAttribute('aria-pressed',String(control.mode==='head'));
      const gated=control.requiresJog&&!control.tracked&&this.state==='playing';$('body-gate').classList.toggle('hidden',!gated);$('body-guide').textContent=this.bodyHasTracked&&!control.tracked?'Return to position':WORDS[status]||'Return to position';$('body-guide-icon').innerHTML=icon(status==='left'||status==='right'?'switch':status==='running'||status==='jog'?'go':'camera');
      [...$('body-meter').children].forEach((el,i)=>el.classList.toggle('on',i<(detail?.steps||0))); 
      ...
      if(control.mode==='body'){paint.save();paint.strokeStyle=control.running?'#c6f578':'#ffce69';paint.setLineDash([4,3]);paint.strokeRect(8,4,144,111);paint.restore();}
      if(head?.points){paint.lineWidth=2;paint.strokeStyle='#c6f578';for(const [a,b] of BODY_LINKS){const pa=head.points[a],pb=head.points[b];if(pa.visibility<.6||pb.visibility<.6)continue;paint.beginPath();paint.moveTo((1-pa.x)*160,pa.y*120);paint.lineTo((1-pb.x)*160,pb.y*120);paint.stroke();}for(const i of [23,24,25,26,27,28]){const p=head.points[i];if(p.visibility<.6)continue;paint.beginPath();paint.arc((1-p.x)*160,p.y*120,2.2,0,Math.PI*2);paint.fillStyle=i>24?'#ffce69':'#c6f578';paint.fill();}return;}
    ...
    $('body-mode').addEventListener('click',()=>control.setMode('body'));$('head-mode').addEventListener('click',()=>control.setMode('head'));$('body-centre').addEventListener('click',()=>control.recalibrate());$('body-keyboard').addEventListener('click',()=>control.disable());
  ...
  start(){this.capture.reset();this.captureTime=0;this.idleSeconds=0;this.standing=false;this.bodyHasTracked=false;this.character.root.rotation.z=0;this.audio.start();this.distance=0;this.score=0;this.tokens=0;this.speed=17;this.time=0;this.threat=0;this.glance=0;this.shake=0;this.pendingDeath=0;this.chain=0;this.lastToken=0;this.newBest=false;this.roofTimer=0;this.accum=0;this.player.reset();for(const k in this.power)this.power[k]=0;this.world.reset(2039+(this.seed++)*919);this.state='playing';this.audio.startMusic();this.ui.state(this.state);this.ui.lastUpdate=1;this.ui.update(0);this.iconToast('go','',.8,'Run started');}
  pause(){if(this.state!=='playing')return;this.state='paused';this.audio.pauseMusic();this.ui.state('paused');this.persist();}
  resume(){this.state='playing';this.audio.resumeMusic();this.ui.state('playing');this.last=performance.now();this.accum=0;}
  menu(){this.capture.reset();this.captureTime=0;this.idleSeconds=0;this.standing=false;this.character.root.rotation.z=0;this.persist();this.audio.stopMusic();this.state='menu';this.distance=0;this.player.reset();this.world.reset(2039);this.world.step(0,this);this.world.renderTokens(this);this.ui.state('menu');this.toastTime=0;$('toast').classList.remove('show');}
  persist(){this.save.data.best=Math.max(this.save.data.best,Math.floor(this.score));this.save.save();}
  end(){if(this.state!=='playing')return;this.newBest=this.score>this.save.data.best;this.persist();this.state='caught';this.captureTime=0;this.captureBase=Math.max(0,this.player.surface);this.captureZ=0;for(const e of this.world.entities){if(Math.abs((e.lane-1)*LANE-this.player.x)<5&&Math.abs(e.s-this.distance)<12){const half=['train','approach','depart','rampTrain'].includes(e.type)?TRAIN_HALF:e.type==='cargo'?1:.5;this.captureZ=Math.max(this.captureZ,this.distance-e.s+half+1.5);}}this.player.y=this.captureBase;this.player.fall=0;this.player.invincible=0;this.player.slide=0;this.ui.state('caught');this.audio.stopMusic();this.audio.play('near');this.toastTime=0;$('toast').classList.remove('show');}
  finishCapture(){this.state='over';this.ui.state('over');this.ui.result();}

  toast(text,sub='',duration=1.5){$('toast').removeAttribute('aria-label');$('toast').innerHTML=`${text}${sub?'<small>'+sub+'</small>':''}`;$('toast').classList.add('show');this.toastTime=duration;}
  iconToast(name,value='',duration=1.2,label=''){this.toast(icon(name)+(value?`<b>${value}</b>`:''),'',duration);$('toast').setAttribute('aria-label',label||name);}
  ...
  activate(type){this.power[type]=this.powerDuration[type];this.stat('powers',1);this.particles.burst(this.player.x,this.player.y+1,0,POWER_COLORS[type],25);this.audio.play('power');this.iconToast(type,'+'+this.powerDuration[type]+'s',1.2,POWER_NAMES[type]+' activated');}
  hit(hard,e){const p=this.player;
    if(this.power.board>0){this.power.board=0;p.invincible=2.2;p.stumble=.4;this.particles.burst(p.x,.3,0,0xe8a4ed,30);this.audio.play('hit');this.iconToast('board','-1',1.2,'Board absorbed impact');this.shake=.18;return;}
    if(this.power.shield>0){this.power.shield=0;p.invincible=2.2;this.particles.burst(p.x,1,0,0x8fd4ff,30);this.audio.play('power');this.iconToast('shield','-1',1.2,'Shield absorbed impact');this.shake=.13;return;}
  ...
  step(dt){if(this.state==='caught'){const before=this.captureTime;this.captureTime+=dt;this.time+=dt;if(before<2.55&&this.captureTime>=2.55)this.audio.play('ui');if(before<2.85&&this.captureTime>=2.85)this.audio.play('power');if(this.captureTime>=CAPTURE_DURATION)this.finishCapture();return;}if(this.state==='playing'&&this.motionControl?.requiresJog&&!this.motionControl.tracked){this.audio.update(dt,0,false);return;}this.time+=dt;this.clock+=dt;
    if(this.state==='playing'){
      this.standing=!!(this.motionControl?.requiresJog&&!this.motionControl.running);
      if(this.motionControl?.tracked)this.bodyHasTracked=true;
      this.idleSeconds=this.standing?(this.idleSeconds||0)+dt:Math.max(0,(this.idleSeconds||0)-dt*2);
      if(this.idleSeconds>=6){this.end();return;}
      if(this.pendingDeath>0){this.pendingDeath-=dt;if(this.pendingDeath<=0){this.end();return;}}
      this.speed+=(difficultyAt(this.distance).speed-this.speed)*dt*1.5;const travel=(this.standing?0:this.speed)*dt*(this.pendingDeath>0?.25:1);this.distance+=travel;this.score+=travel*(this.speed/17)*this.multiplier*(this.power.boost>0?2:1);this.stat('totalDistance',travel);this.threat=Math.max(0,this.threat-dt);
      for(const k in this.power){if(this.power[k]>0){this.power[k]=Math.max(0,this.power[k]-dt);if(this.power[k]===0){this.audio.play('expire');this.iconToast(k,'0',.8,POWER_NAMES[k]+' expired');}}}
      ...
      this.audio.update(dt,this.standing?0:this.speed,this.player.grounded&&this.player.slide<=0&&this.power.board<=0);
  ...
  render(dt){const trackingHold=this.state==='playing'&&this.motionControl?.requiresJog&&!this.motionControl.tracked;if(trackingHold)dt=0;const p=this.player,menu=this.state==='menu',active=this.state==='playing';
    ...
    if(this.rig){
      ...
      this.rig.pose(this.state==='caught'||(this.state==='over'&&this.captureTime)?'caught':this.standing?'idle':this.state,p,this.power,this.time,this.speed,this.glance,this.captureTime);this.rig.update(this.state==='over'?0:dt);
    ...
    this.drone.visible=menu||this.time<4||(this.threat>0||this.idleSeconds>0)||this.state==='over';const droneZ=this.idleSeconds>0?5.2-4*Math.min(1,this.idleSeconds/6):this.state==='over'?1.1:this.threat>0?2.5:5.2;this.drone.position.x+=((menu?-.8:p.x+.7)-this.drone.position.x)*(1-Math.exp(-dt*5));this.drone.position.y=(menu?3.6:2.6)+Math.sin(this.time*3)*.12;this.drone.position.z=menu?0:droneZ;this.drone.rotation.z=Math.sin(this.time*4)*.08;
    this.drone.userData.animate?.(this.time);
    let camX,camY,camZ,lookX,lookY,lookZ;
    // Chase camera. The runner sits low in frame and the horizon sits high,
    // so most of the screen is the track ahead. What buys that is the
    // DEPRESSION ANGLE on the runner -- roughly 31 degrees here, against the
    // 22 it used to be. Raising the camera would get the same angle but
    // shrink him, so the distance comes down instead: closer and steeper,
    // not higher. The near look target (6m ahead, not 13) is what pitches
    // the view down far enough to lift the vanishing point to the top fifth.
    if(menu){camX=-.6+Math.sin(this.time*.12)*.15;camY=3.6;camZ=10;lookX=-2;lookY=1.8;lookZ=-9;}else{camX=p.x*.22;camY=5.8+p.y*.4+Math.sin(this.time*this.speed*.7)*.025;camZ=7.6+(this.speed-17)*.05;lookX=p.x*.22;lookY=1.06+p.y*.3;lookZ=-6;}
    const ease=1-Math.exp(-dt*(menu?2:5));this.camera.position.lerp(new T.Vector3(camX,camY,camZ),ease);this.look.lerp(new T.Vector3(lookX,lookY,lookZ),ease);if(active&&this.shake>0){this.camera.position.x+=(Math.random()-.5)*this.shake*.28;this.camera.position.y+=(Math.random()-.5)*this.shake*.2;}this.camera.lookAt(this.look);const fov=menu?54:61+(this.speed-17)*.38;this.camera.fov+=(fov-this.camera.fov)*ease;this.camera.updateProjectionMatrix();
    if(this.state==='caught'||(this.state==='over'&&this.captureTime))this.capture.update(this,Math.min(CAPTURE_DURATION,this.captureTime));
    this.world.renderTokens(this);this.renderer.render(this.scene,this.camera);
  }
  frame(now){const gated=this.state==='playing'&&this.motionControl?.requiresJog&&!this.motionControl.tracked;$('body-gate').classList.toggle('hidden',!gated);if(gated&&this.bodyHasTracked)$('body-guide').textContent='Return to position';$('idle-warning').classList.toggle('hidden',this.state!=='playing'||!this.standing||gated);$('idle-count').textContent=Math.max(0,Math.ceil(6-(this.idleSeconds||0)));const dt=Math.min((now-this.last)/1000,.1);this.last=now;this.audio.updateMusic(dt);if(!this.testClock&&this.state!=='paused'&&this.state!=='over'){this.accum+=dt;while(this.accum>=1/120){this.step(1/120);this.accum-=1/120;}}if(!document.hidden&&this.state==='playing'&&!this.testClock){this.frameAverage=(this.frameAverage||16)*.97+Math.min(dt*1000,60)*.03;this.qualityFrames=(this.qualityFrames||0)+1;if(this.qualityFrames>180&&this.frameAverage>22&&(this.qualityScale||1)>.7){this.qualityScale=Math.max(.7,(this.qualityScale||1)-.1);this.qualityFrames=0;window.dispatchEvent(new Event('resize'));}}this.render(dt);requestAnimationFrame(this.frame);}
